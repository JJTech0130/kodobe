...
local pok, nginx_c = pcall(require, "openssl.auxiliary.nginx_c")
...
if pok and not os.getenv("CI_SKIP_NGINX_C") then
  ...
  if ngx_version == 1017008 or ngx_version == 1019003 or ngx_version == 1019009 
    ...
    error("openssl.auxiliary.nginx doesn't support Nginx version " .. ngx_version, 2)
  end
  ...
  local NO_C_MODULE_WARNING_MSG = "note openssl.auxiliary.nginx is using plain FFI " ..
  ...
  elseif ngx_lua_version < 10019 then
    ...
    error("openssl.auxiliary.nginx doesn't support lua-nginx-module version " .. (ngx_lua_version or "nil"), 2)
  end
