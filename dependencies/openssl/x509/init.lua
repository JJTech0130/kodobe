...
require "openssl.include.x509v3"
require "openssl.include.evp"
require "openssl.include.objects"
local stack_macro = require("openssl.include.stack")
local stack_lib = require("openssl.stack")
local asn1_lib = require("openssl.asn1")
local digest_lib = require("openssl.digest")
local extension_lib = require("openssl.x509.extension")
local pkey_lib = require("openssl.pkey")
local util = require "openssl.util"
local txtnid2nid = require("openssl.objects").txtnid2nid
local find_sigid_algs = require("openssl.objects").find_sigid_algs
...
local version = require("openssl.version")
...
local function digest(self, cfunc, typ, properties)
  -- TODO: dedup the following with openssl.digest
  local ctx
...
function _M:get_pubkey()
  ...
  local lib = require("openssl.pkey")
...
function _M:get_subject_name()
  ...
  local lib = require("openssl.x509.name")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_subject_alt_name()
  ...
  local lib = require("openssl.x509.altname")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_info_access()
  ...
  local lib = require("openssl.x509.extension.info_access")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_crl_distribution_points()
  ...
  local lib = require("openssl.x509.extension.dist_points")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
